...
public class FakeRepositorioOrdenadores : IRepositorio<Ordenador>
{
    ...
    public void Create(Ordenador item)
    {
        var idNueva = ordenadores.Count;
        item.Id = idNueva;
        ordenadores.Add(item);
    }
    ...
    public void Update(Ordenador item)
    {
        if (item != null)
        {
            var ordenadorSinActualizar = ordenadores.Find(x => x.Id == item.Id);
            if (ordenadorSinActualizar != null)
            {
                ordenadorSinActualizar.Componentes = item.Componentes;
                ordenadorSinActualizar.CalorTotal = item.CalorTotal;
                ordenadorSinActualizar.Precio = item.Precio;
                ordenadorSinActualizar.PedidoId = item.PedidoId;
                ordenadorSinActualizar.Propietario = item.Propietario;
            }
        }

        if (item != null) ordenadores.Add(item);
    }
